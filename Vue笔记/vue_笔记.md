...
* Vue对象的选项
	...
	* computed
	...
	methods和computed的区别：
		methods里面定义的函数大多都是需要事件触发来调用的，而computed中的函数是根据data中的数据变化自动计算出来的，不需要咱们手动调用
	* watch
		* 包含多个属性监视的对象
		* 分为一般监视和深度监视
		  ```
		  ...
		  ```
  * components,filters,directives,分别可以自定义组件，过滤器和指令。  
* **注意**:
    * 上面的三个指令都是需要加s  
    * 使用Vue定义全局的组件，过滤器，表达式的时候一定要写在局部Vue对象的最前面
* 扩展数组方法
